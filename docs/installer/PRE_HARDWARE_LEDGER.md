...
observation that no amount of source code can supply. **A real Windows guest has
now been installed exactly once, for one of the two profiles.** That single
observation closes nothing on its own: PH-12 says base images, plural, and
reproducibility is a claim about repeated runs rather than about one.

PH-08, PH-09, PH-16, and PH-19 still need a guest that boots *from* the built
image; PH-12 needs the second profile's image and a repeated build; PH-13
...
| PH-11 | 12 of 12 resolved for the Windows 11 profile. The Windows 10 profile's 3 base-image-scoped inputs await its image |
| PH-12 | The Windows 11 image is built and re-verified from the disk. The Windows 10 image is unbuilt, and one build of one profile is not reproducibility |
