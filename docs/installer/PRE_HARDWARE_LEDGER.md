...
now been installed for both profiles.** That closes less than it appears to:
PH-12 also asks for reproducibility, which is a claim about repeated builds of
one profile, and each has been built exactly once.

PH-08, PH-09, PH-16, and PH-19 still need a guest that boots *from* the built
image; PH-12 needs a repeated build of one profile; PH-13
...
| PH-12 | Both images are built and re-verified from their disks. Each profile has been built exactly once, and reproducibility is a claim about repeated builds |
