...
| Confirmation, journal, and cross-OS handoff objects | A1 | Deterministic schemas and generated examples bind plan, staging evidence, state-model digest, actor, sequence, and prior record. | A4 | Implement durable signed or authenticated runtime journal replicas, reconciliation, and cross-OS persistence. |
| Same-stream privileged destination transactions | A0 | The public staging API prevents a generic trusted copy and retains an open verified source handle. The required transaction rule is documented. | A4 | Implement destination-specific XBOOTLDR, ESP, and deployment transactions that verify while writing, flush before reachability, and reconcile temporary objects. |
