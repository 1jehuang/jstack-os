...
| Confirmation, journal, and cross-OS handoff objects | A3 for the private file-backed mutation journal; A1 for cross-OS replication | Deterministic schemas and generated examples bind plan, staging evidence, state-model digest, actor, sequence, and prior record. The Unix-host VM simulator persists canonical framed intent, commit, and state-advance records, rejects forks and malformed committed frames, truncates only torn uncommitted tails, revalidates committed outputs on replay, and advances only to the exact target state declared by the executable graph. | A4 | Implement durable authenticated journal replicas on the selected Windows and Linux filesystems, integrate them with the production graph controller, and prove cross-OS reconciliation. |
